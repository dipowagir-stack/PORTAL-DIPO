...
import { ShieldAlert, DownloadCloud, School, LogOut } from 'lucide-react';
...
export default function AccessDenied({ reason }: { reason: string }) {
  ...
  switch (reason) {
    case 'PLATFORM_ACCESS_DENIED':
      title = 'Akses Platform SaaS Ditolak';
      message = 'Halaman Platform HUB SaaS ini dibatasi khusus untuk Platform Administrator (romeoelins.2003@gmail.com). Akun Anda bukan administrator platform.';
      break;
    case 'NO_MEMBERSHIP':
        ...
        {reason === 'PLATFORM_ACCESS_DENIED' && (
          <div className="space-y-3 mb-6">
            <button
              ...
              className="w-full bg-indigo-600 text-white font-medium py-3 px-4 rounded-lg hover:bg-indigo-700 transition-colors flex items-center justify-center shadow-sm"
            >
              <School className="w-5 h-5 mr-2" />
              Masuk ke Portal Sekolah Saya (/app)
            </button>
            <button
              onClick={async () => {
                await logout();
              ...
              className="w-full bg-gray-100 text-gray-700 font-medium py-2.5 px-4 rounded-lg hover:bg-gray-200 transition-colors flex items-center justify-center text-sm"
            >
