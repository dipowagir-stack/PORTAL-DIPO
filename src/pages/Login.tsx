...
export default function Login({ tenantProfile }: { tenantProfile?: TenantPublicProfile }) {
  ...
  if (user && profile && !error) {
    ...
    const isAuthorizedPlatform = isPlatformAdmin || isPlatformTeam;

    // 1. Platform Admin always routed strictly to SaaS Platform HUB
    if (isAuthorizedPlatform) {
      return <Navigate to="/platform" replace />;
    }

    // 2. Tenant Users (schools, teachers, students, admins) strictly routed to Tenant Portal (/app)
    const targetTenantId = tenantProfile?.id || profile.tenantId;
    if (targetTenantId) {
      localStorage.setItem(`activeTenant_${user.uid}`, targetTenantId);
    }
  ...
  if (user && !profile && !error) {
    if (tenantProfile) {
      ...
      // Global login but no profile -> show error and logout
      logout().then(() => {
        setError('Akun Anda tidak ditemukan di sistem. Silakan login melalui portal sekolah Anda.');
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // On success, we wait for AuthContext to detect the user and trigger a redirect.
      // We set a small timeout to clear the loading state just in case the redirect doesn't happen.
      setTimeout(() => {
        if (mounted.current) {
          setIsLoggingIn(false);
        }
