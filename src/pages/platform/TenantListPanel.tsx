...
export default function TenantListPanel() {
  ...
  if (selectedTenant) {
    return <TenantDetailView tenant={selectedTenant} onBack={() => setSelectedTenant(null)} onUpdate={loadTenants} handleSuspend={handleSuspend} handleRestore={handleRestore} />;
  }
...
function TenantDetailView({ tenant: initialTenant, onBack, onUpdate, handleSuspend, handleRestore }: { tenant: TenantSummary, onBack: () => void, onUpdate: () => void, handleSuspend: (id: string) => void, handleRestore: (id: string) => void }) {
  const { profile } = useAuth();
  const isPlatformAdmin = profile?.role === 'platform_admin';
    ...
    <div className="space-y-6">
       ...
       <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-3">
             <button onClick={() => window.open(`https://${tenant.code}.ai.studio`, '_blank')} className="px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50">Manage</button>
             {isPlatformAdmin && (
               <>
                 {tenant.status === 'ACTIVE' ? (
                    <button onClick={() => handleSuspend(tenant.id)} className="px-4 py-2 bg-red-50 border border-red-200 text-red-700 rounded-lg text-sm font-medium hover:bg-red-100">Suspend</button>
                 ) : (
                    <button onClick={() => handleRestore(tenant.id)} className="px-4 py-2 bg-green-50 border border-green-200 text-green-700 rounded-lg text-sm font-medium hover:bg-green-100">Restore</button>
                 )}
               </>
             )}
          </div>
