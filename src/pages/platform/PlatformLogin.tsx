...
export default function PlatformLogin() {
  ...
  // If user is logged in but not an authorized platform admin, sign them out.
  useEffect(() => {
    if (user && profile && !loading) {
      ...
      if (!isAuthorizedPlatform) {
        logout().then(() => {
          setError('Akses Ditolak: Portal SaaS Platform HUB hanya dapat diakses oleh Platform Administrator (romeoelins.2003@gmail.com). Silakan login melalui portal sekolah Anda.');
        }).catch(err => {
