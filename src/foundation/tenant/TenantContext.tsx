...
export const TenantProvider = ({
  ...
  const [entitlement, setEntitlement] = useState<TenantEntitlement | null>(
    null,
  );
  const [loading, setLoading] = useState(true);

  // Platform user detection (strictly authorized platform admin or platform staff)
  const isPlatformUser = useMemo(() => {
    const cleanEmail = (user?.email || "").toLowerCase().trim();
    return (
      (cleanEmail === "romeoelins.2003@gmail.com" &&
      ...
      try {
        ...
        if (isMounted) {
          ...
          const isApplicantPortal =
            pathname.startsWith("/ppdb") || pathname.startsWith("/applicant");
          ...
          if (isPlatformPortal) {
            ...
            if (!targetTenantId) {
              ...
              if (!targetTenantId && profile?.tenantId) {
                targetTenantId = profile.tenantId;
              }
            }

            const validMembership =
              userMemberships.find((m) => m.tenantId === targetTenantId) ||
            ...
            if (validMembership || (isPlatformUser && targetTenantId)) {
              const tenantIdToLoad = validMembership
                ? validMembership.tenantId
                : targetTenantId!;
              const tenantDoc = await getDoc(
                doc(db, "tenants", tenantIdToLoad),
              );
              if (tenantDoc.exists() && tenantDoc.data().status === "ACTIVE") {
                ...
                const entRes = await entitlementService.getTenantEntitlement(
                  resolvedTenant.id,
                  tempSecCtx,
                );
                if (entRes.isSuccess) {
